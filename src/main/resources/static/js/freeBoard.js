...
if (modifyButton) {
	modifyButton.addEventListener("click", event => {
		...
		if (title == "" && content == "") {
			...
			// URL에 실제 freeNo 값을 대체하여 요청을 보냅니다.
			fetch(`/api/freeboards/${freeNo}`, {
				method: "PUT",
				body: formData,
			})
...
commentList.addEventListener('click', (event) => {
	...
	if (deleteButton.classList.contains('cmt-delete-btn')) {
		...
		if (commentItem) { // 댓글 요소를 찾은 경우에만 실행
			...
			const freeCmtNoElement = commentItem.querySelector('.comment-no');
...
document.addEventListener("DOMContentLoaded", function() {
	...
	document.querySelectorAll('.edit-comment-btn').forEach(button => {
		button.addEventListener('click', () => {
			...
			if (!isNaN(freeCmtNo)) { // 파싱한 결과가 NaN이 아닌 경우에만 사용
				editingCommentNo = freeCmtNo;
			}
		});
	});
		...
		button.addEventListener('click', () => {
			...
			if (button.classList.contains('save-comment-btn') && editingCommentNo !== null) {
				...
				fetch(`/api/freecomments/${editingCommentNo}`, { // 수정 중인 댓글 번호 사용
					...
					body: JSON.stringify({ freeCmtContent: updatedContent })
				})
					.then(response => {
						if (response.status === 200) {
...
let freeCmtContent = document.getElementById('freeCmtContent');
let counterCmt = document.getElementById('counterCmt');
freeCmtContent.addEventListener('input', function() {
	countingLength('freeCmtContent', 'counterCmt');
});

let editfreeCmtContent = document.getElementById('editfreeCmtContent');
let counterEdit = document.getElementById('counterEdit');
editfreeCmtContent.addEventListener('input', function() {
	countingLength('editfreeCmtContent', 'counterEdit');
});